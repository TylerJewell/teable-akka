...
const nextConfig = {
  ...
  async rewrites() {
    ...
    // The interface is unchanged except for where it gets its data: every /api call and
    // the socket handshake are sent to the backend named by TEABLE_API_ORIGIN instead of to
    // this process, which used to serve them itself.
    const apiOrigin = process.env.TEABLE_API_ORIGIN;
    if (apiOrigin) {
      // beforeFiles, because /api is also a pages directory here and a filesystem route
      // wins over an afterFiles rewrite -- which serves the 404 page to every data call.
      return {
        beforeFiles: [
          { source: '/api/:path*', destination: `${apiOrigin}/api/:path*` },
          { source: '/socket/:path*', destination: `${apiOrigin}/socket/:path*` },
        ],
      };
    }
